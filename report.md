...
- **Self-RAG**：给模型加“是否需要检索/检索是否相关”的反思判断，减少不必要的检索与幻觉回答。
- **CRAG**：当检索质量差时触发“纠错路径”，比如降级为关键词搜索或提示用户补充信息。
- **Adaptive-RAG**：先做问题复杂度判断，简单问题少检索、复杂问题多步检索。

### 2. 多智能体与任务分解

- **Agentic RAG**：引入 Router / Retriever / Grader / Answerer 的分工，提升复杂问题准确率。
- **ReAct**：在对话中展示“推理 + 工具调用”的过程（可选隐藏给后台），提升可解释性。

### 3. 结构化知识增强

- **GraphRAG（轻量版）**：抽取“部门-职责-流程”关系形成轻量知识图谱，用于流程类问题。
- **Parent-Child 分块**：小片段用于精准召回，大片段用于生成上下文（agentic-rag-for-dummies 的思路）。

### 4. 可用性与可靠性增强

- **查询澄清**：当问题含糊时先追问（human-in-the-loop）。
- **多路召回**：向量 + 关键词 + 元数据（文档类型/来源）融合。
- **评估与监控**：引入离线评测集，记录“回答采纳率、误答率、检索失败率”。

### 5. Java 生态可选技术栈

- **LangChain4j**：Java 版 LangChain，支持对话记忆、工具调用与向量检索。
- **Spring AI**：Spring 官方 AI 框架，兼容多模型 + 向量数据库。
- **Milvus / Weaviate / PGVector**：均有 Java 客户端，可作为向量存储备选。

### 6. 可扩展的产品化方向

- **文档权限体系**：按用户/角色配置可见文档（已实现基础版）。
- **对话持久化**：对话历史入库，便于审计与持续优化（已实现基础版）。
- **个性化知识库**：按学院/年级/培养方案区分索引。
- **流程卡片化**：对常见问题生成流程图/清单，辅助学生快速操作。

### 7. 论文思路可落地的具体功能

- **RAG-Fusion**：先生成多条查询重写，再融合多路检索结果提升召回。
- **HyDE**：先生成“假想答案”作为检索查询，再回到真实文档获取依据。
- **RAPTOR**：通过层次化摘要/聚类，支持“从宏观到细节”的检索。
- **ColBERT / SPLADE**：稀疏+密集融合检索，可提升精确度与可解释性。

### 8. Java 生态可参考的开源项目

- **langchain4j**：Java LLM 编排与工具调用。
- **spring-ai**：Spring 官方 AI 生态集成（模型、向量库、RAG）。
- **djl**：Java 深度学习库，可加载本地 embedding 或 reranker。
- **open-search-java / elasticsearch-java**：向量检索与混合检索支持。
